...
export default function POSApp() {
  ...
  const loadProducts = useCallback(async () => {
    if (!merchant) return;
    const res = await fetch(`${API}/api/merchants/${merchant.id}/products`);
  ...
  const loadInvoices = useCallback(async () => {
    if (!merchant) return;
    const res = await fetch(`${API}/api/merchants/${merchant.id}/invoices`);
    const data = await res.json();
  ...
  async function addProduct() {
    if (!merchant) return;
    setLoading(true);
    await fetch(`${API}/api/merchants/${merchant.id}/products`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  if (!merchant) {
    return (
      <div style={{ maxWidth: 480, margin: "80px auto", padding: 20, textAlign: "center" }}>
        ...
        <a href="http://localhost:3002"><Button>Open Merchant Portal →</Button></a>
      </div>
    );
  }
